...
(a) Suppose $x _0$ =1 and $x_1$ =2 and

$$ x_{j}=x_{j-i}+\frac{2}{x_{j-1}} $$
for $j=1,2,...$
Write a function \texttt testLoop which takes the single argument $n$ and returns the first $n − 1$ values of the sequence $x_{j}_{j≥0}$: that means the values of $x_0$, $x_1$, $x_2$, . . . , $x_{n}−2$.
...
for $r=2$ and $0 < x1$ you should get $xn -> 0.5$ as $n -> infinity$. 
...
(b)Now write a function which determines the number of iterations needed to get $|xn − x_{(n−1)}| < 0.02$. So this function has only 2 arguments: start and rho. (For $start=0.95$ and $rho=2.99$, the answer is 84.)
